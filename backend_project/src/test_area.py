...
# import numpy as np
# import copy
# from src.convert_to_walls import convert_all_rooms_to_walls
...
import numpy as np
from collections import defaultdict,Counter
# from convert_to_walls import convert_all_rooms_to_walls
...
# irregular_constraints = {
#     'Balcony': {'min_width': 2, 'min_height': 2}
# }

irregular_constraints = {
    'Master Bedroom': {'min_width': 4, 'min_height': 4},
    'Kitchen': {'min_width': 4, 'min_height': 4},
    'Living Room': {'min_width': 4, 'min_height': 4},
    'Bedroom': {'min_width': 4, 'min_height': 4},
    'Foyer': {'min_width': 2, 'min_height': 2},
    'Dining Room': {'min_width': 4, 'min_height': 4},
    'En suite Washroom': {'min_width': 2, 'min_height': 2},
    'Common Washroom': {'min_width': 2, 'min_height': 2},
    'Passage': {'min_width': 2, 'min_height': 2},
    'Bathroom': {'min_width': 2, 'min_height': 2},
    'Washroom': {'min_width': 2, 'min_height': 2},
    'Balcony': {'min_width': 2, 'min_height': 2},
    'MB Passage': {'min_width': 2, 'min_height': 2}
}


merged_constraints = constraints.copy()
merged_constraints.update(constraints1)
merged_constraints.update(passage_constraints)
merged_constraints.update(irregular_constraints)
...
def sanitize_dimensions(dimensions):
    for room, dim in dimensions.items():
        dim['width'] = float(dim['width'])
        dim['height'] = float(dim['height'])
    return dimensions
...
def calculate_area(coords):
    ...
    return round(0.5 * abs(sum((x[i] * y[(i + 1) % len(y)] - y[i] * x[(i + 1) % len(x)]) for i in range(len(coords)))), 2)

def recalculate_coordinates(rooms, dimensions):
    new_rooms = {}
    for room, coordinates in rooms.items():
        all_points = [pt for wall in coordinates for pt in wall]
        unique_points = list({(pt[0], pt[1]) for pt in all_points})
        centroid_x, centroid_y = calculate_polygon_centroid(unique_points)

        scale_x = dimensions[room]['scaled_width'] / dimensions[room]['width']
        scale_y = dimensions[room]['scaled_height'] / dimensions[room]['height']

        scaled_coordinates = []
        for wall in coordinates:
            scaled_wall = []
            for (x, y) in wall:
                new_x = centroid_x + (x - centroid_x) * scale_x
                new_y = centroid_y + (y - centroid_y) * scale_y
                scaled_wall.append([
                    truncate_to_two_decimals(new_x),
                    truncate_to_two_decimals(new_y)
                ])
            scaled_coordinates.append(scaled_wall)

        new_rooms[room] = scaled_coordinates
    return new_rooms
...
def calculate_area_percentages(dimensions):
    ...
    for room, dim in dimensions.items():
        area = round(dim['width'] * dim['height'], 2)
        dim['area'] = area
        dim['percentage'] = round((area / total_area) * 100, 2)
    return dimensions
...
def make_roomdata(rooms):
    ...
    wall_counter = 1
    for room, walls in rooms.items():
        unique_points = list({(pt[0], pt[1]) for wall in walls for pt in wall})
        room_area = calculate_area(unique_points)
        wall_numbers = defaultdict(list)
        ...
        for wall in walls:
            ...
            metadata.setdefault(room, {})[wall_counter] = {
                ...
                'wall_length': calculate_wall_length(start, end),
                'wall_type': wall_type,
                'room_area': room_area,
                'parallel_walls': [],
                'direction': 'Unknown',
        ...
        for wall_type, ids in wall_numbers.items():
            for i in ids:
                metadata[room][i]['parallel_walls'] = ids
    return metadata
...
# def dynamic_area_calculater(rooms, target_total_area):
#     rooms = sanitize_rooms(rooms)
#     room_dimensions = calculate_dimensions(rooms)
#     room_dimensions = sanitize_dimensions(room_dimensions)
#     room_dimensions = calculate_area_percentages(room_dimensions)
#     room_dimensions = scale_dimensions(room_dimensions, target_total_area)
#     new_rooms = recalculate_coordinates(rooms, room_dimensions)
#     metadata = make_roomdata(new_rooms)
#     return new_rooms, metadata

def validate_room_dimensions(metadata, constraints):
    errors = []
    for room, data in metadata.items():
        width = max(coord[0] for wall in data.values() for coord in wall['coordinates']) - \
                min(coord[0] for wall in data.values() for coord in wall['coordinates'])
        height = max(coord[1] for wall in data.values() for coord in wall['coordinates']) - \
                 min(coord[1] for wall in data.values() for coord in wall['coordinates'])

        constraint = constraints.get(room)
        if constraint:
            min_w = constraint.get('min_width', 0)
            min_h = constraint.get('min_height', 0)
            max_w = constraint.get('max_width', float('inf'))
            max_h = constraint.get('max_height', float('inf'))

            if not (min_w <= width <= max_w):
                errors.append(f"{room} width {width:.2f} out of bounds ({min_w}-{max_w})")
            if not (min_h <= height <= max_h):
                errors.append(f"{room} height {height:.2f} out of bounds ({min_h}-{max_h})")
    return errors
...
# Modify dynamic_area_calculater to replace near values
...
def adjust_room_dimensions_to_meet_constraints(metadata, constraints, fixed_room_dimensions, rooms):
    """
    This function adjusts the dimensions of rooms after scaling to meet the min/max width/height constraints.
    """
    ...
    for room, data in metadata.items():
        ...
        if constraint:
            ...
            # Fix width if needed
            if width < min_w:
                scale_factor = min_w / width
                _scale_room(metadata, room, scale_factor, axis='x')
            elif width > max_w:
            ...
            if height < min_h:
                scale_factor = min_h / height
                _scale_room(metadata, room, scale_factor, axis='y')
            elif height > max_h:
                scale_factor = max_h / height
                _scale_room(metadata, room, scale_factor, axis='y')

def _scale_room(metadata, room, scale_factor, axis='x'):
    """
    Internal helper to scale a room along x or y axis by a scale_factor.
    """
    for wall_id, wall in metadata[room].items():
        start, end = wall['coordinates']

        if axis == 'x':  # Scale horizontally
            start[0] = start[0] * scale_factor
            end[0] = end[0] * scale_factor
        elif axis == 'y':  # Scale vertically
            start[1] = start[1] * scale_factor
            end[1] = end[1] * scale_factor

        wall['coordinates'] = [start, end]
        wall['wall_length'] = calculate_wall_length(start, end)
...
def dynamic_area_calculater(rooms, target_total_area):
    ...
    # ✅ RESIZE here after scaling!
    adjust_room_dimensions_to_meet_constraints(metadata, merged_constraints, {}, new_rooms)

    return new_rooms, metadata



def generate_floorplan_main(rooms: dict, total_area: float, flat_type: str):
    if (flat_type == '1BHK' and total_area < 300) or (flat_type == '2BHK' and total_area < 525):
        print(f"❌ Error: Given area {total_area} is not sufficient for a {flat_type}.")
        return {}
    updated_rooms, metadata = dynamic_area_calculater(rooms, total_area)
    errors = validate_room_dimensions(metadata, merged_constraints)
    ...
    wall_segments = convert_all_rooms_to_walls(updated_rooms)
    return wall_segments
...
def main():
    ...
    wall_segments = generate_floorplan_main(rooms, total_area, flat_type)
    print("\n✅ Final Wall Segments:")
...
# def generate_floorplan_main(rooms, total_area, flat_type):
#     if (flat_type == '1BHK' and total_area >= 300) or (flat_type == '2BHK' and total_area >= 525):
#         print("rooms",rooms)
#         updated_rooms, metadata = dynamic_area_calculater(rooms, total_area)
#         print("updated_room :",updated_rooms)
#         wall_segments = convert_all_rooms_to_walls(updated_rooms)
#         print("wall_segments :",wall_segments)
#         return wall_segments
#     else:
#         print(f"❌ Error: Given area {total_area} is not sufficient for {flat_type}.")
#         return {}
