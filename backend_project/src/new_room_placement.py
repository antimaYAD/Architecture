...
def make_roomdata(rooms):
    ...
    for room, walls in rooms.items():
        room_area = truncate_to_two_decimals(calculate_area(walls))
        total_area += room_area
        # wall_numbers = {'horizontal': [], 'vertical': []} 
        wall_numbers = {'horizontal': [], 'vertical': [], 'diagonal': []}

        #  # Separate wall numbers by type
...
def find_adjacent_rooms(room_name, rooms):
    ...
    adjacent_rooms = defaultdict(list)
    target_lines = [LineString(wall) for wall in rooms[room_name]]
    ...
    for other_room, walls in rooms.items():
        ...
        for wall in walls:
            ...
            if any(w.intersects(wall_line) for w in target_lines):
                direction = determine_direction(rooms[room_name], walls)
                adjacent_rooms[direction].append(other_room)
                break

    for dir in adjacent_rooms:
        adjacent_rooms[dir] = list(set(adjacent_rooms[dir]))
...
def calculate_dimensions_from_metadata(room_data):
    all_coords = [pt for wall in room_data.values() for pt in wall.get('coordinates', [])]
...
def skip_if_opposite_walls_share_same_axis(walls1, walls2):
    if len(walls1) != 4 or len(walls2) != 4:
        return False  # Can't apply logic safely
...
def add_irregular_room(room_data, room_name, vertices):
    """
    Define an irregular room using ordered polygon vertices.
    Converts vertices into connected walls (line segments).
    """
    walls = []
    n = len(vertices)
    for i in range(n):
        walls.append([vertices[i], vertices[(i + 1) % n]])  # loop closure
    room_data[room_name] = walls
    return room_data
...
def convert_to_polygon(room_coords):
    points = [pt for wall in room_coords for pt in wall]
    poly = Polygon(points)
...
def find_and_calculate_overlaps(rooms_coords, area_threshold=0.1):
    overlaps = {}
        ...
        for room2, coords2 in rooms_coords.items():
            ...
            if poly1.intersects(poly2):
                ...
                if intersection.area > area_threshold:
                    ...
                    overlaps[(room1, room2)] = (truncate_to_two_decimals(intersection.area), direction)
    return overlaps
...
def generate_updated_floorplan(
    rooms: dict,
    new_room_name: str,
    length: float,
    width: float,
    ...
    partial_overlaps = find_and_calculate_overlaps(updated_rooms)

    if partial_overlaps:
        resolved_rooms = solving_overlap_problem(
            partial_overlaps, updated_rooms, original_rooms, new_room_name
        )
        metadata = make_roomdata(resolved_rooms)

        gap_results = find_all_gaps(resolved_rooms)
        update_instructions = create_update_dicts(
            gap_results, new_room_name=new_room_name, metadata=metadata
        )
        apply_updates(update_instructions, metadata)
        ...
        broken_connections, broken_rooms = compare_floor_plans(
            original_rooms, updated_rooms, new_room_name
        )
        final_broken_info = remove_duplicate_connections(broken_connections)
        final_broken_info = filter_broken_connections_due_to_new_room(
            final_broken_info, new_room_name, updated_rooms
        )

        shift_analysis_dict = make_shift_analysis_dict(
            final_broken_info.keys(), updated_rooms, original_rooms
        )
    ...
    final_rooms = replace_near_values(final_rooms, threshold=0.3)

    # ✅ Get all wall segments as one flat list
    converted_walls = convert_all_rooms_to_walls(final_rooms)
    print(converted_walls)
    # walls_segments =converted_walls

    return {
        "updated_rooms": final_rooms,
        "converted_walls": converted_walls
    }
